...
| `count_values()` aggregation | Supported, evaluated in the proxy. VictoriaLogs has no construct that groups by a metric's computed values, so the inner query runs and the proxy buckets the resulting series by their sample value: one output series per distinct value, labelled `{<label>="<value>"}`, valued by how many input series carried it (Prometheus/Loki semantics; input labels are dropped). Cost scales with the inner query's series count, not with rows. Not to be confused with grouping by a log field's values, which is `sum by (<field>) (count_over_time(...))`. |
