...
| Manual range-metric window bounds | The manual (proxy-side) range aggregation closes its window on both ends, where LogQL's range vector at time `t` selects `(t-range, t]`. A sample landing exactly on a bucket edge is therefore counted in two adjacent buckets: on 20 s-spaced data in 1 m tumbling buckets that is +33 % against Loki. Affects every query on the manual path (`rate`, `count_over_time`, the unwrap family), not only the template path. Not yet fixed — `aggregateManualWindow` in `internal/proxy/range_metric_compat.go`. |
