...
- #1 ci: build to the maestra ECR, drop upstream release/publish/compat/security/docs workflows.
- #2 feat: `-field-mapping` fallback chains, `-computed-labels`, derived level, mapped labels in results, `-line-field=_msg`.
- #3 fix: emit LogsQL VictoriaLogs accepts for derived level.
- #5 fix: honour range-aggregation grouping on the instant path; serve `count_values`.
- #6 ci(fuzz): budget fuzz-smoke by exec count.
- #12 chore: Go 1.27.1, module/image/action bumps, dependabot.
- #13 fix: level grouping without raw-row scans; chain-label grouping; per-step topk.
- #14 feat: `line_format`/`label_format` Go templates and `__error__` filters in the proxy.
- #15 fix(range): buckets labelled by their END on the Loki grid; rate under topk; `or vector(0)`; row-guard on empty results.
- #16 fix: Loki grid on the grouped range path, Loki quantile semantics, no extracted-label shadowing.
- #17 fix: drop the raw grouping key for mapped labels, snap the step grid, restore comparison filtering.
- #18 fix: range-vector window, compound durations, or-filters, spaced calls, pod-label mapping, `detected_level` from text.
- #19 fix(metrics): stream the raw-row scan, translate binary operands, bucketed fold on Loki's grid.
- #20 fix: stored vs derived level (`-derived-level-fields`/`-derived-level-group-by`), whole-stage unpack strip, Loki's level set, per-point windows, real row cap.
- #21 fix(logsql): quote every embedded regexp, bound the logs-path sort, deterministic pod-label aliases.
- #22 fix: real limit pushdown, honest series cap, level operators, coprime grid, `sum(rate)` pooling, `label_format` alias, template unwrap route, `-msg-field-aliases`.
- #23 fix: `-line-filter-fields` fan-out, empty instant vectors, offset, pattern, bare range identity.
- #24 ci: org-wide cross-account pull policy on the ECR repository.
- #25 fix: level unpack keeps stored fields, `Entry.SplitJSON` for `| json` on collector-split rows, lossless stage re-quoting, `completeStreamIdentity` for bare range aggregations.
